...
public class AlphaBeta {
    ...
    public static void computeAndPlayBestMove(int[][] game, int player) {
        ...
        if (player == X) {
            result = maxTurn(game, 0, Integer.MIN_VALUE, Integer.MAX_VALUE);
        } else {
            result = minTurn(game, 0, Integer.MIN_VALUE, Integer.MAX_VALUE);
        }
    ...
    static Result maxTurn(int[][] game, int depth, int alpha, int beta) {
        if (isTerminalState(game)) {
        ...
        for (int i = 0; i < game.length; ++i) {
            for (int j = 0; j < game[i].length; ++j) {
                if (game[i][j] == E) {
                    ...
                    Result currentMove = minTurn(game, depth + 1, alpha, beta);
    ...
    static Result minTurn(int[][] game, int depth, int alpha, int beta) {
        if (isTerminalState(game)) {
            return new Result(score(game, depth), null);
        }
        ...
        for (int i = 0; i < game.length; ++i) {
            for (int j = 0; j < game[i].length; ++j) {
                if (game[i][j] == E) {
                    ...
                    // Recurse for other moves
                    Result currentMove = maxTurn(game, depth + 1, alpha, beta);
